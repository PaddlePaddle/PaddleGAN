...
DEFINE_string(config_dir, "", "Path of inference model config");
...
void PredictVideo(const std::string& video_path,
  ...
  while (capture.read(frame)) {
    ...
    vsr->Predict(imgs, 0, 1, &result, &det_times);
...
int main(int argc, char** argv) {
  ...
  PaddleGAN::VSR vsr(FLAGS_model_dir, FLAGS_config_dir, FLAGS_device, FLAGS_use_mkldnn,
  ...
  PredictVideo(FLAGS_video_file, &vsr, FLAGS_output_dir);
  return 0;
}
