...
def parse_args():
    ...
    parser.add_argument(
        '--run_process_type', type=str, default="SingleP", help='MultiP or SingleP')
    parser.add_argument(
...
class TimeAnalyzer(object):
    ...
    def _get_fps(self, mode, base_batch_size, gpu_num, avg_of_records, run_process_type, unit=None):
        if mode == -1 and run_process_type == 'SingleP':
            assert unit, "Please set the unit when mode is -1."
            fps = gpu_num * avg_of_records
        elif mode == -1 and run_process_type == 'MultiP':
            assert unit, "Please set the unit when mode is -1."
            fps = gpu_num * avg_of_records  # temporarily, not used now
            print("------------this is MultiP")
        elif mode == 0:
            # s/step -> samples/s
    ...
    def analysis(self, base_batch_size, gpu_num=1, skip_steps=0, mode=-1, run_process_type='SingleP', unit=None):
        ...
        fps, fps_unit = self._get_fps(mode, base_batch_size, gpu_num, avg_of_records, run_process_type, unit)
        fps_skipped, _ = self._get_fps(mode, base_batch_size, gpu_num, avg_of_records_skipped, run_process_type, unit)
        if mode == -1:
...
if __name__ == "__main__":
    ...
    run_info["run_process_type"] = args.run_process_type
