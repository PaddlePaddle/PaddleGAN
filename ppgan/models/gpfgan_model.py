...
@MODELS.register()
class GPFGANModel(BaseModel):
    """ This class implements the gfpgan model.

    """
    def __init__(self, **opt):

        super(GPFGANModel, self).__init__()
        self.opt = opt
        train_opt = opt
        ...
        if self.use_identity:
            ...
            self.nets['network_identity'] = build_discriminator(
                self.opt['network_identity'])
            load_path = self.opt['path'].get('pretrain_network_identity')
            if load_path is not None:
                load_val = paddle.load(load_path)
                self.nets['network_identity'].set_state_dict(load_val)
            self.nets['network_identity'].eval()
            for param in self.nets['network_identity'].parameters():
    ...
    def train_iter(self, optimizers=None):
        ...
        if (self.current_iter % self.net_d_iters == 0
            ...
            if self.use_identity:
                ...
                # # print(gt_gray.shape)
                identity_gt = self.nets['network_identity'](gt_gray).detach()
                identity_out = self.nets['network_identity'](out_gray)
