...
class gpf_FaceEnhancement(object):
    def __init__(self,
                 GPFGAN_weights,
                 path_to_enhance=None,
                 size=512,
                 batch_size=1):
        super(gpf_FaceEnhancement, self).__init__()

        # Initialise the face detector
        if path_to_enhance is None:
            # model_weights_path = get_path_from_url(GPFGAN_weights)
            model_weights = paddle.load(GPFGAN_weights)
        else:
